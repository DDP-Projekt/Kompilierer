...
func Compile(file string, src []byte, errorHandler scanner.ErrorHandler) (*CompileResult, error) {
	var Ast *ast.Ast
	var err error
...
func CompileTo(file string, src []byte, errorHandler scanner.ErrorHandler, writer io.Writer) (*CompileResult, error) {
	...
	result, err := New(Ast, errorHandler).Compile(writer)
	return result, err
}

// compile the given AST to llvm ir
func CompileAst(Ast *ast.Ast, errorHandler scanner.ErrorHandler) (*CompileResult, error) {
	return New(Ast, errorHandler).Compile(nil)
}
