...
func (c *Compiler) setupListTypes() {
	...
	c.declareInbuiltFunction("_ddp_{{ .T }}_equal", ddpbool, ir.NewParam("list1", {{ .T }}ptr), ir.NewParam("list2", {{ .T }}ptr))
	c.declareInbuiltFunction("_ddp_{{ .T }}_slice", {{ .T }}ptr, ir.NewParam("list", {{ .T }}ptr), ir.NewParam("index1", ddpint), ir.NewParam("index2", ddpint))
	c.declareInbuiltFunction("_ddp_{{ .T }}_to_string", ddpstrptr, ir.NewParam("list", {{ .T }}ptr))

	c.declareInbuiltFunction("_ddp_{{ .T }}_{{ .T }}_verkettet", {{ .T }}ptr, ir.NewParam("list1", {{ .T }}ptr), ir.NewParam("list2", {{ .T }}ptr))
	c.declareInbuiltFunction("_ddp_{{ .T }}_{{if .D}}ddpstring{{else}}{{ .E }}{{end}}_verkettet", {{ .T}}ptr, ir.NewParam("list", {{ .T }}ptr), ir.NewParam("el", {{if .D}}ddpstrptr{{else}}{{ .E }}{{end}}))
	{{if .D}}
	...
	{{else}}
	c.declareInbuiltFunction("_ddp_{{ .E }}_{{ .E }}_verkettet", {{ .T }}ptr, ir.NewParam("el1", {{ .E }}), ir.NewParam("el2", {{ .E }}))
	c.declareInbuiltFunction("_ddp_{{ .E }}_{{ .T }}_verkettet", {{ .T }}ptr, ir.NewParam("el", {{ .E }}), ir.NewParam("list", {{ .T }}ptr))
	{{end}}
	{{end}}
}
...
case {{ .T }}ptr:
	switch rhs.Type() {
	case {{ .T }}ptr:
		c.latestReturn = c.cbb.NewCall(c.functions["_ddp_{{ .T }}_{{ .T }}_verkettet"].irFunc, lhs, rhs)
	case {{if .D}} ddpstrptr {{else}} {{ .E }} {{end}}:
		c.latestReturn = c.cbb.NewCall(c.functions["_ddp_{{ .T }}_{{if .D}}ddpstring{{else}}{{ .E }}{{end}}_verkettet"].irFunc, lhs, rhs)
	default:
...
case {{ .E }}:
	switch rhs.Type() {
	case {{ .E }}:
		c.latestReturn = c.cbb.NewCall(c.functions["_ddp_{{ .E }}_{{ .E }}_verkettet"].irFunc, lhs, rhs)
	case {{ .T }}ptr:
		c.latestReturn = c.cbb.NewCall(c.functions["_ddp_{{ .E }}_{{ .T }}_verkettet"].irFunc, lhs, rhs)
	default:
		err("invalid Parameter Types for VERKETTET (%s, %s)", lhs.Type(), rhs.Type())
	}
{{end}}
