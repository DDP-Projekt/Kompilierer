...
#define ALLOCATE(type, count) \
    (type*)_ddp_reallocate(NULL, 0, sizeof(type) * (count))

// helper macro to free any type (not arrays though)
#define FREE(type, pointer) _ddp_reallocate(pointer, sizeof(type), 0)
...
#define GROW_ARRAY(type, pointer, oldCount, newCount) \
    (type*)_ddp_reallocate(pointer, sizeof(type) * (oldCount), \
        sizeof(type) * (newCount))

// helper to free a whole array
#define FREE_ARRAY(type, pointer, oldCount) \
    _ddp_reallocate(pointer, sizeof(type) * (oldCount), 0)
