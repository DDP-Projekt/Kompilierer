...
func (p *parser) strictParam(assignable bool) (ast.Expression, *ddperror.Error) {
	start := p.cur
	which := -1
	...
	case token.NEGATE:
		...
		if !p.matchAny(token.INT, token.FLOAT, token.IDENTIFIER, token.SYMBOL) {
			ddperr := ddperror.New(ddperror.SYN_UNEXPECTED_TOKEN, ddperror.LEVEL_ERROR, p.previous().Range, ddperror.MsgGotExpected(p.previous(), "ein Zahlen Literal", "ein Kommazahlen Literl", "ein einzelnes Token"), p.module.FileName)
			return nil, &ddperr
		}
		which = 1
	case token.LPAREN:
		...
		if p.atEnd() {
			ddperr := ddperror.New(ddperror.SYN_UNEXPECTED_TOKEN, ddperror.LEVEL_ERROR, p.previous().Range, ddperror.MsgGotExpected(token.EOF, ")"), p.module.FileName)
			return nil, &ddperr
		}
		which = 2
	default:
		ddperr := ddperror.New(ddperror.SYN_UNEXPECTED_TOKEN, ddperror.LEVEL_ERROR, p.peek().Range, ddperror.MsgGotExpected(p.peek(), "einen Wert"), p.module.FileName)
		return nil, &ddperr
	}
	tokens := make([]token.Token, p.cur-start)
	copy(tokens, p.tokens[start:])
	...
	if assignable {
		...
		return argParser.orErr(func() ast.Expression {
			return argParser.assigneable()
		})
	}
	...
	case 0:
		return argParser.orErr(func() ast.Expression {
			return argParser.primary(nil)
		})
	case 1:
		return argParser.orErr(func() ast.Expression {
			return argParser.negate()
		})
	case 2:
		return argParser.orErr(func() ast.Expression {
			argParser.advance()
			return argParser.grouping()
		})
	default:
...
func (p *parser) new_alias() ast.Expression {
	...
	parsedAliasParams := make(map[*token.Token]ast.Expression, 8)
	parsingCache := make(map[int]cachedToken, 30)
	...
	matchedAlias, matchedKeys, ok := p.aliases.Find(func(node_index int, tok *token.Token) (*token.Token, bool) {
		// the if statement below is a more efficient map[int]int implementation
		// abusing the fact that node_index is incremental
		if node_index == len(start_indices)-1 { // key is on top of the map
		...
		} else if node_index < len(start_indices)-1 { // key is not on top of the map, so the rest has to be cleared
			start_indices = start_indices[:node_index+1]
			p.cur = start_indices[node_index]
		} else if node_index >= len(start_indices) { // key is not in the map
		...
		case token.COMMA:
			keyEnds[tok] = p.cur + 1
			return p.advance(), !p.atEnd()
		case token.ALIAS_PARAMETER:
			break
		default:
			p.matchAny(token.COMMA)
			keyEnds[tok] = p.cur + 1
			return p.advance(), !p.atEnd()
		}

		// tok.Type == token.ALIAS_PARAMETER

		cachedTok := parsingCache[p.cur]

		cur := p.cur
		var finalExpr ast.Expression
		var typ ddptypes.Type
		if tok.AliasInfo.IsReference {
			if cachedTok.ass == nil {
				if !p.matchAny(token.IDENTIFIER, token.LPAREN) {
					keyEnds[tok] = p.cur + 1
					return p.advance(), !p.atEnd()
				}

				var ass ast.Expression
				var ddperr *ddperror.Error

				if node_index == 0 {
					p.decrease()
					ass, ddperr = p.strictParam(tok.AliasInfo.IsReference)
				} else {
					ass, ddperr = p.inAliasMode(func() ast.Expression {
						return p.assigneable()
					})
				}
			...
			finalExpr = cachedTok.expr
		}

		p.cur = cachedTok.end

		typ = cachedTok.assType
		if typ == nil {
			typ = p.typechecker.EvaluateSilent(finalExpr) // evaluate the argument
		}

		if tok.AliasInfo.IsReference {
			cachedTok.assType = typ
		} else {
			cachedTok.exprType = typ
		}

		parsingCache[cur] = cachedTok

		didMatch := true
		if !ddptypes.Equal(typ, tok.AliasInfo.Type) {
			didMatch = false
		} else if ass, ok := finalExpr.(*ast.Indexing);                                        // string-indexings may not be passed as char-reference
		tok.AliasInfo.IsReference && ddptypes.Equal(tok.AliasInfo.Type, ddptypes.BUCHSTABE) && // if the parameter is a char-reference
		...
		parsedAliasParams[tok] = finalExpr
		keyEnds[tok] = p.cur
		return tok, true
	}, aliasGreater)
	...
	aliasToks := matchedAlias.GetTokens()
	for i, tok := range matchedKeys {
		if tok.Type != token.ALIAS_PARAMETER {
			continue
		}
		argName := strings.Trim(aliasToks[i].Literal, "<>") // remove the <> from the alias parameter
		arg, ok := parsedAliasParams[tok]
		if !ok {
			panic("did not match arg Name")
		}
		args[argName] = arg
	}
	...
	p.cur = keyEnds[matchedKeys[len(matchedKeys)-1]]
	return callOrLiteralFromAlias(matchedAlias, args)
...
func (p *parser) strict_alias() ast.Expression {
	...
	sort.Slice(matchedAliases, func(i, j int) bool {
		return aliasGreater(matchedAliases[i], matchedAliases[j])
	})
...
func aliasGreater(a, b ast.Alias) bool {
	toksa, toksb := a.GetTokens(), b.GetTokens()
	if len(toksa) != len(toksb) {
		return len(toksa) > len(toksb)
	}
	...
	refNi, refNj := countRefArgs(a.GetArgs()), countRefArgs(b.GetArgs())
	return refNi > refNj
}
