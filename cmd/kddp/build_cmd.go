...
var buildCmd = &cobra.Command{
	Use:   "kompiliere [-o <Ausgabe Datei>] [--main <main.o Datei>] [--gcc-flags <GCC Flags>] [--extern-gcc-flags <GCC Flags>] [--nodeletes] [--verbose] [--link-modules] [--link-list-defs] [--gcc-executable <Pfad zu gcc>] <Dateien>...",
	Short: "Kompiliert eine oder mehrere .ddp Datei",
	Long:  `Kompiliert eine oder mehrere .ddp Datei in ausführbare, llvm oder objekt Dateien.`,
	Args:  cobra.MinimumNArgs(1),
	RunE: func(cmd *cobra.Command, args []string) error {
		// for multiple files, the output path is flag is ignored
		if len(args) > 1 {
			buildOutputPath = ""
		}
...
func init() {
	buildCmd.Flags().StringVarP(&buildOutputPath, "ausgabe", "o", "", "Optionaler Pfad der Ausgabedatei. Bei mehrern Eingabe Dateien wird dieser ignoriert")
